...
| JSON 键 | Session 键 | 说明 |
|---------|------------|------|
| `workspace_tab` | `ui_workflow` | `潮滩推理` / `GEE 数据下载`（支持别名） |
...
| `root_dir` 等路径 | `ui_root_dir`, `ui_mask_root`, … | 路径类字符串 |
...
| type | 写入 | 说明 |
|------|------|------|
| `run_pipeline` | `pending_task` | DL / 指数法推理 |
| `run_m4` | `pending_task` (mode=m4) | GEE 下载 |
| `run_autotune` | `pending_autotune` | 自适应调参 |

### 口语 → JSON 映射（System Prompt 已注入）

- **例 A**：「深度学习跑 24zhejiang，概率 5% 频次 2」→ `run_mode=dl`, `prob_th=0.05`, `min_cnt=2`, `pending_action=run_pipeline`
- **例 B**：「打开 E1，参考师姐 2020」→ `e1_enabled=true`, `e1_reference=师姐_2020`
- **例 C**：「开 M5，指数法跑一下」→ `m5_enabled=true`, `run_mode=index`, `pending_action=run_pipeline`
- **例 D**：「切下载页，云量 20，启动 M4」→ `workspace_tab=GEE数据下载`, `m4_cloud=20`, `pending_action=run_m4`
